...
class NameNERRunner:
    def __init__(self, model_path="models/name_ner_bert", artifacts_dir="name_ner_bert_artifacts", local_files_only=False):
        ...
        print(f"{Fore.YELLOW}Model path: {self.model_path}")
        print(f"{Fore.YELLOW}Local files only: {self.local_files_only}")
        
        # Load model and tokenizer
        try:
            print(f"{Fore.CYAN}Loading tokenizer...")
            ...
            if self.local_files_only:
                print(f"{Fore.YELLOW}💡 Try running without local_files_only flag or check your local model files")
            sys.exit(1)
